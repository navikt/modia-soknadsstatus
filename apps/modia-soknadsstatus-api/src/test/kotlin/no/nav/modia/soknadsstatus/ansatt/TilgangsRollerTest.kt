...
class TilgangsRollerTest {
    ...
    fun `skal sette opp produksjonsroller når klusteret er i prod`() {
        ...
        assertEquals(sensitiveTilgangsRoller.skjermedePersoner.gruppeId.get(), SKJERMEDE_PERSONER_PROD, "Id for kode7 gruppe i prod var ikke riktig.")
        assertTrue(NASJONALE_TILGANGER_PROD == geografiskeTilgangsRoller.nasjonaleTilgangsRoller.map { it.gruppeId.get() }, "Nasjonale tilganger i prod var ikke riktig.")
        assertTrue(REGIONALE_TILGANGER_PROD == geografiskeTilgangsRoller.regionaleTilgangsRoller.map { it.gruppeId.get() }, "Regionale tilganger i prod var ikke riktig.")
    }
    ...
    fun `skal sette opp preprod roller når klusteret er i dev`() {
        ...
        assertEquals(sensitiveTilgangsRoller.skjermedePersoner.gruppeId.get(), SKJERMEDE_PERSONER_PREPROD, "Id for skjermede personer gruppe i preprod var ikke riktig.")
        assertTrue(NASJONALE_TILGANGER_PREPROD == geografiskeTilgangsRoller.nasjonaleTilgangsRoller.map { it.gruppeId.get() }, "Nasjonale tilganger i preprod var ikke riktig.")
        assertTrue(REGIONALE_TILGANGER_PREPROD == geografiskeTilgangsRoller.regionaleTilgangsRoller.map { it.gruppeId.get() }, "Regionale tilganger i preprod var ikke riktig.")
    }
    ...
    fun `skal sette opp prod roller når kluster ikke er satt`() {
        ...
        assertEquals(sensitiveTilgangsRoller.kode6.gruppeId.get(), KODE6_PROD, "Id for kode6 gruppe i prod var ikke riktig.")
        assertEquals(sensitiveTilgangsRoller.kode7.gruppeId.get(), KODE7_PROD, "Id for kode7 gruppe i prod var ikke riktig.")
        assertEquals(sensitiveTilgangsRoller.skjermedePersoner.gruppeId.get(), SKJERMEDE_PERSONER_PROD, "Id for skjermede personer gruppe i prod var ikke riktig.")
        assertTrue(NASJONALE_TILGANGER_PROD == geografiskeTilgangsRoller.nasjonaleTilgangsRoller.map { it.gruppeId.get() }, "Nasjonale tilganger i prod var ikke riktig.")
        assertTrue(REGIONALE_TILGANGER_PROD == geografiskeTilgangsRoller.regionaleTilgangsRoller.map { it.gruppeId.get() }, "Regionale tilganger i prod var ikke riktig.")
    }
}
