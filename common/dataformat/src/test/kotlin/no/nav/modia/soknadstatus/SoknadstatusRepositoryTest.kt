...
class SoknadstatusRepositoryTest {
    ...
    val repository: SoknadstatusRepository by lazy { SoknadstatusRepository(dataSource) }
    ...
    fun `should update existing record if newer`() {
        ...
        repository.upsert(
            dummyOppdatering.copy(
                status = SoknadstatusDomain.Status.FERDIG_BEHANDLET,
        ...
        assertSuccess(repository.get(ident)) { oppdateringer ->
            assertEquals(1, oppdateringer.size)
            assertEquals(SoknadstatusDomain.Status.FERDIG_BEHANDLET, oppdateringer.first().status)
        }
    }
    ...
    fun `should ignore updates older then current saved state`() {
        ...
        assertSuccess(repository.get(ident)) { oppdateringer ->
            assertEquals(1, oppdateringer.size)
            assertEquals(SoknadstatusDomain.Status.UNDER_BEHANDLING, oppdateringer.first().status)
        }
    }
